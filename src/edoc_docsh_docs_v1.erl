...
-module(edoc_docsh_docs_v1).

-behaviour(edoc_docsh_format).
-export([lookup/3]).

-behaviour(edoc_docsh_writer).
...
-type item() :: {KNA :: edoc_docsh_format:kna(),
                 Anno :: erl_anno:anno(),
                 Signature :: [binary()],
...
-spec lookup(Docs, Key, Kinds) -> R when
      Docs  :: edoc_docsh_format:t(),
      Key   :: edoc_docsh_format:key(),
      Kinds :: edoc_docsh_format:kinds(),
      R :: {ok, [item()]}
         | {not_found, edoc_docsh_format:error_message()}.
lookup(#docs_v1{} = Docs, Key, Kinds0) ->
    %edoc_docsh_lib:print("lookup ~p ~p in\n  ~p\n", [Key, Kinds0, Docs]),
...
dispatch_lookup(Docs, _Mod, [moduledoc]) ->
    case Docs#docs_v1.module_doc of
        none   -> {not_found, edoc_docsh_format:module_doc_not_available()};
        hidden -> {not_found, edoc_docsh_format:module_doc_hidden()};
        Doc    -> {ok, [Doc]}
    end;
dispatch_lookup(Docs, {_Mod, Name, Arity}, [doc, type]) ->
    case fetch_items(Docs#docs_v1.docs, type, Name, Arity) of
        []    -> {not_found, edoc_docsh_format:item_doc_not_available()};
        Items -> {ok, Items}
    end;
...
-spec from_internal(edoc_docsh_internal:t()) -> t().
from_internal(Internal) ->
    %% TODO: remove
    %edoc_docsh_lib:print("internal ~p\n", [Internal]),
    ...
    %edoc_docsh_lib:print("Docs : ~p\n", [Docs]),
    %edoc_docsh_lib:print("DocsMap: ~p\n", [DocsMap]),
...
step(ModuleInfo, Info, { #docs_v1{} = DocsV1, DocsMap }) ->
    %edoc_docsh_lib:print("item: ~p\n", [Info]),
    %ct:pal("item: ~p\n", [Info]),
    {_, Name, Arity} = KNA = edoc_docsh_internal:kna(Info),
    Entry = { KNA,
              erl_anno:new({0, 1}),
...
kna_signature(ModuleInfo, Info) ->
    #{name := Mod} = ModuleInfo,
    {_, Name, Arity} = edoc_docsh_internal:kna(Info),
